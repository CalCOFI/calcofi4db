...
#'
#' @param data_info List containing data and metadata from load_csv_files()
#'
...
transform_data <- function(data_info) {
  ...
  mutate_table <- function(tbl, data) {
    ...
    y <- dplyr::rename_with(data, ~ f_rn[.x])
...
#' @param d_flds_rn Field redefinition data frame
#'
...
#' )
#' }
detect_csv_changes <- function(con, schema, transformed_data, d_flds_rn) {
