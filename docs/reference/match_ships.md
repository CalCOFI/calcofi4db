...
and ship_name, followed by word-overlap fuzzy matching against CalCOFI
UNOLS, NODC WOD, and optionally ICES reference sources. Manual overrides
can be provided via a CSV file.
...
- unmatched_ships:
  ...
  tibble with columns: ship_code, ship_name (ships needing
  reconciliation from bottle casts)

- reference_ships:

  tibble with columns: ship_key, ship_nodc, ship_name (known ships from
  swfsc database)

- ship_renames_csv:

  optional path to manual overrides CSV with columns: csv_name,
  csv_code, csv_name_new, csv_code_new
