...
Checks that all values in a foreign key column exist in the referenced
table's primary key column. Returns a tibble of orphan rows that fail
the check.
...
Tibble with orphan rows (rows where fk_col value not in
ref_tbl.ref_col). Contains all columns from data_tbl where the reference
fails.
