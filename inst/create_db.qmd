...
params:
  ...
  version_description: "Addition of CalCOFI Bottle Database"
  do_version_release: true
  do_git_commit: true
  do_git_push: true
  do_copy_to_prod: true
---
...
Render this Quarto document from command line (while in root of the calcofi4db directory and with Postgres database connected):

```bash
cd inst
quarto render create_db.qmd
```
...
librarian::shelf(
  # CalCOFI/calcofi4db, 
  DBI, dm, dplyr, DT, duckdb, fs, glue, gargle, googledrive, here, janitor,
  jsonlite, knitr, lubridate, purrr, readr, rlang, rlang, tibble, tidyr, uuid,
  quiet = T)
...
dk_db     <- glue("{dir_data}/calcofi.duckdb")

# get database connections
con            <- get_db_con(c("dev", "dev_ref")) # for reference
con_dev        <- get_db_con(c("dev"))            # for dev only
con_dev_public <- get_db_con(c("dev", "public"))  # for spatial
con_dk         <- dbConnect(duckdb(), dbdir = dk_db, read_only = F)

# dbListTables(con_dk) |> sort()
...
dbExecute(con_dev_public, "UPDATE dev.site SET geom = ST_SetSRID(ST_MakePoint(longitude, latitude), 4326)") # 61,104 rows
```
...
librarian::shelf(
  CalCOFI/calcofi4r, dplyr, mapview, sf, tidyr, units)
...
## TODO: transfer db-viz-hex/taxa_worms.qmd workflow

- notebook: [taxa_worms.html](https://calcofi.io/db-viz-hex/taxa_worms.html)
- source: [taxa_worms.qmd](https://github.com/CalCOFI/db-viz-hex/blob/main/taxa_worms.qmd)

Tables modified (∆) or added (+) by:

- [ ] ∆ species
- [ ] + taxonomy
- [ ] + taxon_rank

## Dataset: CalCOFI Bottle Database
...
dir_ingest   <- here(glue("inst/ingest/{provider}/{dataset}"))

bottle_csv   <- glue("{dir_dataset}/{subdir}/194903-202105_Bottle.csv")
cast_csv     <- glue("{dir_dataset}/{subdir}/194903-202105_Cast.csv")
...
stopifnot(all(file.exists(c(bottle_csv, cast_csv))))
...
# write d_cast and d_bottle to dev database
dbWriteTable(con_dev, "cast", d_cast, overwrite = T)
dbWriteTable(con_dev, "bottle", d_bottle, overwrite = T)
```
...
if (params$do_version_release) {
  ...
  release_result <- complete_version_release(
    ...
    additional_files = c(
      ...
      "inst/ingest/swfsc/ichthyo/flds_redefine.csv",
      "inst/ingest/swfsc/ichthyo/tbls_redefine.csv"
    ),
  ...
  permalink <- glue("https://github.com/CalCOFI/calcofi4db/blob/{git_hash}/inst/create_db.qmd")

  # unlink(here("inst/schema_version.csv")); dbExecute(con_dev, "TRUNCATE TABLE dev.schema_version")
...
2. **Run the notebook** - it will:
   ...
   - Copy `dev` schema to `prod` (if enabled)
...
# copy dev schema to prod after successful version recording
if (params$do_copy_to_prod) {
  ...
  # display summary
  result$summary |>
    datatable(caption = "Schema comparison: dev vs prod")

} else {

  message(glue("
    ═══════════════════════════════════════════════════════════════
    Skipping Copy to Production
    ═══════════════════════════════════════════════════════════════
    Set params$do_copy_to_prod = TRUE to enable copying dev to prod
    ═══════════════════════════════════════════════════════════════
  "))
...
tbls = dbListTables(con_dev) |> sort()
#  [1] "cruise"   "egg"      "grid"     "larva"    "net"      "ship"     "site"     "site_seg"
#  [9] "species"  "tow"      "tow_type"
# tbls = dbListTables(con_dev) |> setdiff(dbListTables(con_dk))

for (tbl in tbls){
  if (!(dbExistsTable(con_dk, tbl)))
    dbReadTable(con_dev, tbl) %>% 
