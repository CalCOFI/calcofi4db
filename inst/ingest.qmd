...
message("Dev schema recreated successfully")
```
...
```{r}
#| label: noaa-check-mismatches

# detect changes between csv files and redefinitions
changes_noaa <- detect_csv_changes(d_noaa)

# print summary statistics
print_csv_change_stats(changes_noaa, verbose = TRUE)

# display interactive table of changes
if (nrow(changes_noaa$summary) > 0) {
  display_csv_changes(changes_noaa, format = "DT", title = "CSV vs Redefinition Mismatches")
}
```
...
if (nrow(changes_noaa$summary) > 0) {
  ...
  cat(glue("
    ...
    - **Tables redefinition**: `{d_noaa$paths$tbls_rd_csv}`
    - **Fields redefinition**: `{d_noaa$paths$flds_rd_csv}`
...
# record schema version with metadata
